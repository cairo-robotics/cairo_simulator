...
def main():
    ...
    fk = sawyer_robot.solve_forward_kinematics(start)
    print(fk[0][0], quat2rpy(fk[0][1]))
    time.sleep(5)
    
    print("################## RANDOM CONFIGURATION. NOTICE THAT THE LAST WRIST ANGLE IS OFFSET ##################")
    sawyer_robot.set_joint_state(random_start_configurations[-1])
    fk = sawyer_robot.solve_forward_kinematics(random_start_configurations[-1])
    print(fk[0][0], quat2rpy(fk[0][1]))
    time.sleep(5)

     
    print("################## AND IK RESULTS USING FK OF START. NOTICE THAT THE LAST WRIST ANGLE IS STILL OFFSET ##################")
    sawyer_robot.set_joint_state(sawyer_robot.solve_inverse_kinematics(start_world_pose[0], start_world_pose[1]))
    fk = sawyer_robot.solve_forward_kinematics(sawyer_robot.solve_inverse_kinematics(start_world_pose[0], start_world_pose[1]))
    print(fk[0][0], quat2rpy(fk[0][1]))
    time.sleep(5)
