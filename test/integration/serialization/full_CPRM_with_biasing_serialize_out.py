...
def main():
    ...
    configurations = []
    data_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), "orientation_data")
    ...
    with DisabledCollisionsContext(sim, [], []):
        ...
        planning_state_space = SawyerTSRConstrainedSpace(robot=sawyer_robot, TSR=tsr, svc=svc, sampler=DistributionSampler(distribution_model=model, fraction_uniform=.25), limits=None)
        # See params for PRM specific parameters robot, tsr, state_space, state_validity_checker, interpolation_fn, params
        prm = CPRM(SawyerCPRMSimContext, config, sawyer_robot, tsr, planning_state_space, tree_state_space, svc, interp, params={
