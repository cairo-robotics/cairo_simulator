...
class DistributionSpace():
    ...
    def __init__(self, sampler, limits):
        """
        Represents a learned distribution space. This could be a keyframe distribution, trajectory distribution, or any arbitrary distrubtion. It utilizes a sampler that contains the learned distribution model.

        The limits argument passed to the sample() method of the sampler is to ensure the sampler returns a point with dimension values that lie within required limits. This could be joint limits for the dimensions representing joints, reachability limits for relative distances etc,.
